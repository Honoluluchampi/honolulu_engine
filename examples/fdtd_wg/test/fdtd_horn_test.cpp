...
TEST(fdtd_horn, ctor) {
  ...
  EXPECT_EQ(45, edge_infos[1].y());
  EXPECT_EQ(60, edge_infos[2].y());
  EXPECT_EQ(522, edge_infos[3].y());
  EXPECT_EQ(522, horn->get_whole_grid_count());

  auto grid_counts = horn->get_grid_counts();
  EXPECT_EQ(5,  grid_counts[0].x());
  EXPECT_EQ(9, grid_counts[0].y());
  EXPECT_EQ(15, grid_counts[1].x());
  EXPECT_EQ(1,  grid_counts[1].y());
  EXPECT_EQ(22, grid_counts[2].x());
  EXPECT_EQ(21, grid_counts[2].y());
}
