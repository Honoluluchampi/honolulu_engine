...
void engine::load_data()
{
  ...
  // load_actor();
    ...
    smooth_vase_model_comp->set_translation(Eigen::Vector3f{0, 0, 0.f});
    smooth_vase_model_comp->set_scale(Eigen::Vector3f{0, 0, 0});
      ...
      for (int i = 0; i < light_colors.size(); i++) {
        ...
        auto light_rotation = Eigen::AngleAxisd(
            ...
            Eigen::Vector3d{0.f, -1.0f, 0.f}).cast<float>(); // axiz
//      // light_comp->set_translation(Eigen::Vector3f(light_rotation * Eigen::Vector3f(-1.f, -1.f, -1.f)));
        light_comp->set_translation(Eigen::Vector3f{0.f, -3.f, 0.f});
...
void engine::load_actor()
{
  ...
  auto smooth_vase_model_comp = std::make_shared<mesh_component>(smooth_vase_mesh_model);
  smooth_vase->set_renderable_component(smooth_vase_model_comp);
  smooth_vase_model_comp->set_translation(Eigen::Vector3f{-0.5f, 0.5f, 0.f});
  smooth_vase_model_comp->set_scale(Eigen::Vector3f{3.f, 1.5f, 3.f});
  ...
  flat_vase_model_comp->set_translation(Eigen::Vector3f{0.5f, 0.5f, 0.f});
  flat_vase_model_comp->set_scale(Eigen::Vector3f{3.f, 1.5f, 3.f});
  ...
  floor_model_comp->set_translation(Eigen::Vector3f{0.f, 0.5f, 0.f});
  floor_model_comp->set_scale(Eigen::Vector3f{3.f, 1.5f, 3.f});
