...
{
  int light_index = 0;
  auto light_rotation = Eigen::AngleAxisd(-dt, Eigen::Vector3d::UnitY()).cast<float>();
  for (auto& kv : light_comp_map_) {
    ...
    ubo_.point_lights[light_index].position << light_comp->get_transform_sp()->translation.cast<float>(), 1.f;
    ubo_.point_lights[light_index].color << light_comp->get_color().cast<float>(), light_comp->get_light_info().light_intensity;
    light_index++;
  }
